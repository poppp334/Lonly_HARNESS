...
The repository includes an offline evaluation framework (`eval/eval_lonly.py`) with **53 acceptance checks**:
...
- **Track R (R1–R8)**: Adversarial red team security suite (shell injection resilience, IPv6/CIDR TargetPolicy, URL parser confusion resistance, below-agent broker authorization, SecretVault token resolution, CapabilityPolicy descriptors, and session log secret redaction).
